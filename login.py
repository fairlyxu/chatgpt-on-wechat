...
def login(username, password, tenant_id, captcha_input):
    ...
    # 发送POST请求进行登录
    response = requests.post(LOGIN_API, json=credentials, headers=headers)
...
# 创建输入行
def create_input_row(label_text, entry_variable,label_width=5,entry_width=20):
    frame = tk.Frame(root)
    label = tk.Label(frame, text=label_text, bg="#f0f0f0", width=label_width, anchor='w',font=font)  # 设置固定宽度
...
# 创建验证码相关的行
def create_captcha_row(label_width=5):
    frame = tk.Frame(root)
    label_captcha = tk.Label(frame, text="验证码",  width=6, anchor='w', font=font)  # 设置固定宽度
    entry_captcha = tk.Entry(frame, width=label_width, font=font)
    label_captcha.pack(side=tk.LEFT, padx=10)
    entry_captcha.pack(side=tk.LEFT, padx=10)
    global label_captcha_img
    label_captcha_img = tk.Label(frame)
    label_captcha_img.pack(side=tk.LEFT, padx=20)
    frame.pack(pady=8)
...
def create_login_window():
    ...
    #
    # from PIL import Image, ImageTk
    # canvas = tk.Canvas(root, bg="white", height=280, width=400)  # 高度改为280以适应窗口
